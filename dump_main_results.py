...
def dataset_level_print(ds_print_name, ds_internal_name, train_print_name, train_internal_name, k, N, Qsigmas,
    ...
    model = f'{ds_internal_name}_quarter_{train_internal_name}_{epoches}_{lr}_st'
    sigma = 0.25
    print('Neyman-Pearson certification')
    nice_print(read_original_radius(model, 'gaussian', k, sigma, N, 0.001), unit=0.25)

    print('DSRS Certification')
    nice_print(read_improved_radius(model, 'gaussian' + ('-th' if ds_internal_name == 'imagenet' else ''), k, sigma, [Qsigmas[0]], N//2, 0.0005), unit=0.25)
    ...
    model = f'{ds_internal_name}_half_{train_internal_name}_{epoches}_{lr}_st'
    sigma = 0.50
    print('Neyman-Pearson certification')
    nice_print(read_original_radius(model, 'gaussian', k, sigma, N, 0.001), unit=0.25)

    print('DSRS Certification')
    nice_print(read_improved_radius(model, 'gaussian' + ('-th' if ds_internal_name == 'imagenet' else ''), k, sigma, [Qsigmas[1]], N//2, 0.0005), unit=0.25)

    print('')

    print(f'Smoothing distribution: generalized Gaussian with k = {k}, sigma = 1.00')
    model = f'{ds_internal_name}_one_{train_internal_name}_{epoches}_{lr}_st'
    sigma = 1.00

    print('Neyman-Pearson certification')
    nice_print(read_original_radius(model, 'gaussian', k, sigma, N, 0.001), unit=0.25)
    print('DSRS Certification')
    nice_print(read_improved_radius(model, 'gaussian' + ('-th' if ds_internal_name == 'imagenet' else ''), k, sigma, [Qsigmas[2]], N//2, 0.0005), unit=0.25)
...
if __name__ == '__main__':
    ...
    dataset_level_print('CIFAR10', 'cifar10', 'Gaussian Augmentation', 'gaussian', None, N, [0.2, 0.4, 0.8],
                        standard_stdout)

    print(""">>>>> Table 2 and Table 7 - CIFAR10 - Consistency """)
    dataset_level_print('CIFAR10', 'cifar10', 'Consistency', 'consistency', None, N, [0.2, 0.4, 0.8],
                        standard_stdout)

    print(""">>>>> Table 2 and Table 7 - CIFAR10 - SmoothMix """)
    dataset_level_print('CIFAR10', 'cifar10', 'SmoothMix', 'smoothmix', None, N, [0.2, 0.4, 0.8],
                        standard_stdout)
    '''
