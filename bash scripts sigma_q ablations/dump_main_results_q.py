...
new_radius_dir = 'data_q/new-radius'
orig_radius_dir = 'data_q/orig-radius'

# place to store the output human-friendly or TeX-friendly tables and figures
result_folder = 'result_q'
...
def dataset_level_print(ds_print_name, ds_internal_name, train_print_name, train_internal_name, k, N, Qsigmas,
    ...
    '''
    print('DSRS Certification')
    q_sigma = [[x/10] for x in range(1,10)]
    for q_sig in q_sigma:
        print(f"Sigma Q = {q_sig}")
        nice_print(read_improved_radius(model, 'general-gaussian' + ('-th' if ds_internal_name == 'imagenet' else ''), k, sigma, q_sig, N//2, 0.0005), unit=0.25)
...
if __name__ == '__main__':
    ...
    dataset_level_print('CIFAR10', 'cifar10', 'Consistency', 'consistency', 1530, N, [0.2, 0.4, 0.8],
                        standard_stdout)
